...
export function LoginModal({ isOpen, onClose }: LoginModalProps) {
    ...
    return (
        ...
        <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 
            ...
            <button onClick={onClose} aria-label="close" className="absolute top-3 right-3 text-gray-500 hover:text-gray-800">
                <FontAwesomeIcon icon={faXmark} />
            </button>
            <h2 className="text-2xl font-semibold mb-4">Log in</h2>
            
            <form onSubmit={handleSubmit(handleLogin)} className="flex flex-col gap-4">
              <input {...register("email")} aria-label="email" placeholder="Your email.." className="bg-white border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-800  dark:bg-gray-500" />
              {errors.email && <p className="text-xs text-red-600 dark:text-red-300 italic">Please insert your @stud.noroff.no email</p>}

              <input {...register("password")} type="password" aria-label="password" placeholder="Your password.." className="bg-white border rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 dark:focus:ring-indigo-800 dark:bg-gray-500" />
