...
export function VenueForm({ defaultValues, onSubmit, submitLabel = "Save"}: venueFormProps) {
    ...
    return (
        <FormProvider {... methods}>
                ...
                <button 
                    type="submit" 
                    className="mt-2 bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 dark:bg-indigo-800 dark:hover:bg-indigo-900 transition shadow-md w-1/2 mx-auto">{submitLabel}</button>
            </form>
        </FormProvider>
    );
}
