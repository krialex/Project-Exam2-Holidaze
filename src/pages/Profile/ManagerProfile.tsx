    ...
    return (
        <div className="flex flex-col gap-4 mx-auto my-6">
            ...
            <button onClick={() => setIsNewVenueOpen(true)}
                className="bg-indigo-600 text-white px-3 py-1 rounded hover:bg-indigo-700 dark:bg-indigo-800 dark:hover:bg-indigo-900 transition text-sm">Create a new venue</button>
            </div>
            <h3 className="text-2xl font-semibold text-center">My venues</h3>
            {venues.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <Link key={v.id} to={`/venues/${v.id}`} className="h-full">
                    ...
                    <p className="text-sm text-gray-600 dark:text-white">{v.bookings?.length ?? 0} Bookings:</p>
                    <div className="mt-3">
                        {v.bookings?.map((b: any) => (
                        <p key={b.id} className="text-xs italic text-gray-700 dark:text-white">
                            Booked from:{" "}
                    ...
                    <div className="flex gap-2 mt-4">
                        <button onClick={(e) => {
                            ...
                            className="mt-auto bg-[#4996FC] text-white px-3 py-1 rounded hover:bg-[#1c73e5] dark:bg-[#1557ae] dark:hover:bg-[#114a94] transition">Edit</button>
                        <button onClick={(e) => {
                            e.preventDefault();
                            e.stopPropagation();
                            setSelectedVenue(v);
                            ...
                            className="mt-auto bg-[#F53232] text-white px-3 py-1 rounded hover:bg-[#c02424] dark:bg-[#ad1414] dark:hover:bg-[#850f0f] transition">Delete</button>
                    </div>
                    </div>
                </Link>
                ))}
